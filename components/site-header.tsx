...
export function SiteHeader() {
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-40 flex w-full items-center justify-center border-b bg-background dark:bg-background/95`}
      >
              ...
              <div className="w-full max-w-xs">
                <div className="relative flex items-center">
                  <Input
                    ...
                    ref={inputRef}
                    autoComplete="off"
                    className="h-8 rounded-full pr-7"
                    placeholder="Search..."
                  />
                  {isLoading && (
                    <motion.div
                ...
                {((searchResult?.data?.length > 0 && focusSearch) ||
                  delayFocusSearch) && (
                  <motion.div
                    initial={{ maxHeight: 0 }}
                    animate={{
                      maxHeight: focusSearch
                        ? "max(200px, calc(100vh - 12rem))"
                        : 0,
                    }}
                    ...
                    {searchResult?.data?.length > 0 &&
                      searchResult?.data.map((item) => (
                        <div
                          key={item.mal_id}
                          onClick={() =>
                            push(`${correctInitialPathname}/${item.mal_id}`)
                          }
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <h2 className="text-xs font-bold transition-all duration-300 group-hover:text-sm">
                              {item.title}
                            </h2>
                            <p className="hidden text-xs opacity-0 transition-all duration-300 group-hover:block group-hover:opacity-50">
                              {item.type}, {item.episodes} eps, scored{" "}
                              {item.score}
                            </p>
                            {!isManga ? (
                            ...
                            <p className="hidden  text-xs opacity-0 group-hover:block group-hover:opacity-50">
                              {item.status}
                            </p>
                            <p className="text-xs opacity-50">
                              {item.genres
